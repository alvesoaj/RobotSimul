...
class Application(ShowBase):
    ...
    def __init__(self):
        ...
        taskMgr.doMethodLater(0.1, self.checkRobotray, "check robotray")
    ...
    def addActor(self):
        ...
        self.ray.setPos(0, -5, 0.3)
        self.ray.setHpr(0, 90, 0)
        # self.ray.setScale(5)
        self.ray.reparentTo(self.robot)

        self.rayCount = 0

        colRobotRay = self.ray.attachNewNode(CollisionNode("robotray"))
        colRobotRay.node().addSolid(CollisionSegment(0, 0.5, 0.3, 0, 1, 0.3))
        colRobotRay.setHpr(0, 90, 0)
        colRobotRay.show()
    ...
    def exitP(self):
        self.im.save(os.getcwd() + "/textures/map.png")
        exit()
    ...
    def checkRobotray(self, task):
        ...
        y = int(self.robot.getY() + self.ray.getY() + 29)

        # matrix[x][y] = 255
        # self.drawChart()

        if self.rayCount >= 5:
            self.ray.setY(self.ray.getY() + 5);
            self.rayCount = 0
